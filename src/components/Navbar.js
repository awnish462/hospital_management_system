        ...
        <nav className="bg-blue-600 text-white p-4 shadow-md">
            <div className="container mx-auto flex justify-between items-center">
                ...
                <div className="flex-grow flex items-center justify-center space-x-4">
                    ...
                    {user && (user.role === 'ROLE_PHARMACIST' || user.role === 'ROLE_ADMIN') && (
                        <Link to="/pharmacy" className="hover:text-blue-200">Pharmacy</Link>
                    )}
                    {user && user.role === 'ROLE_ADMIN' && (
                        <Link to="/admin/users" className="hover:text-blue-200">User Management</Link>
                ...
                <div>
                    ...
                    <button onClick={handleLogout} className="bg-red-500 hover:bg-red-600 px-3 py-2 rounded font-semibold">Logout</button>
                </div>
            </div>
        </nav>
    );
